...
class MpcKeyManager {
     ...
     * @return A list of KeyShares distributed to parties.
     */
    fun generateDistributedKey(threshold: Int, totalShares: Int): List<KeyShare> {
        ...
        val shares = mutableListOf<KeyShare>()
        for (i in 1..totalShares) {
            val shareData = ByteArray(32)
            secureRandom.nextBytes(shareData)
            shares.add(KeyShare(i, Base64.getEncoder().encodeToString(shareData)))
        }
        return shares
    }
