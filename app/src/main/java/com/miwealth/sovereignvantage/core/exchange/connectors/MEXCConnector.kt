    ...
    override fun parseOrder(response: JsonObject): ExecutedOrder? {
        return try {
            ...
            ExecutedOrder(
                ...
                executedQuantity = response.get("executedQty")?.asString?.toDoubleOrNull() ?: 0.0,
                executedPrice = response.get("cummulativeQuoteQty")?.asString?.toDoubleOrNull()?.let { cumQty ->
                ...
                feeCurrency = "",
                timestamp = response.get("time")?.asLong ?: System.currentTimeMillis(),
