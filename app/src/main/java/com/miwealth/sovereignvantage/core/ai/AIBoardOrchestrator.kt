...
/**
 * AI Board of Directors - The "Octagon" Architecture (8 Members) - V5.5.62
 * 
...
data class MarketContextSnapshot(
    ...
    val high24h: Double,
    val low24h: Double,
    // Macro sentiment fields (V5.5.51)
...
data class BoardConsensus(
    ...
    /** V5.6.0: Board-recommended position size multiplier (0.0–1.0). 
...
data class MarketContext(
    ...
    // V5.5.89: Sentiment data (populated when SentimentEngine is wired)
...
// ============================================================================
// V5.5.89: DQN FEATURE VECTOR BUILDER
 ...
 * V5.5.89: All 19 base features computed from real indicators (not reconstructed from buckets).
...
fun buildDQNFeatures(context: MarketContext, currentPosition: Double = 0.0): Pair<EnhancedFeatureVector, Double> {
    ...
    // V5.5.89: Social volume — use real data if wired, else derive proxy from volume spikes
    ...
    // V5.5.89: News impact — use real data if wired, else derive proxy from price shocks
 ...
 * Weight: 12.5% (Octagon)
 * V5.5.80: Now DQN-augmented - blends learned Q-values with EMA/MACD/ADX analysis
 */
class TrendFollower(private val dqn: DQNTrader? = null) : BoardMember {
    ...
    override fun analyze(context: MarketContext): AgentOpinion {
        ...
        // V5.5.80: Blend with DQN learned patterns if available
 ...
 * Weight: 12.5% (Octagon)
 * V5.5.80: Now DQN-augmented - blends learned patterns with RSI/BB mean reversion signals
 */
class MeanReverter(private val dqn: DQNTrader? = null) : BoardMember {
    ...
    override fun analyze(context: MarketContext): AgentOpinion {
        ...
        // V5.5.81: Blend with DQN learned reversal patterns if available
        val (finalSentiment, finalConfidence, dqnInsight) = if (dqn != null) {
            try {
...
class VolatilityTrader(private val dqn: DQNTrader? = null) : BoardMember {
    ...
    override fun analyze(context: MarketContext): AgentOpinion {
        ...
        // V5.5.81: Blend with DQN learned volatility exploitation patterns
        val (finalSentiment, finalConfidence, dqnInsight) = if (dqn != null) {
            try {
...
class SentimentAnalyst(private val dqn: DQNTrader? = null) : BoardMember {
    ...
    override fun analyze(context: MarketContext): AgentOpinion {
        ...
        // V5.5.81: Blend with DQN learned crowd psychology patterns
        val (finalSentiment, finalConfidence, dqnInsight) = if (dqn != null) {
            try {
...
class OnChainAnalyst(private val dqn: DQNTrader? = null) : BoardMember {
    ...
    override fun analyze(context: MarketContext): AgentOpinion {
        ...
        // V5.5.81: Blend with DQN learned whale/accumulation patterns
        val (finalSentiment, finalConfidence, dqnInsight) = if (dqn != null) {
            try {
...
class MacroStrategist(private val dqn: DQNTrader? = null) : BoardMember {
    ...
    override fun analyze(context: MarketContext): AgentOpinion {
        ...
        // V5.5.81: Blend with DQN learned macro regime patterns
        val (finalSentiment, finalConfidence, dqnInsight) = if (dqn != null) {
            try {
...
class PatternRecognizer(private val dqn: DQNTrader? = null) : BoardMember {
    ...
    override fun analyze(context: MarketContext): AgentOpinion {
        ...
        // V5.5.81: Blend with DQN learned pattern reliability
        val (finalSentiment, finalConfidence, dqnInsight) = if (dqn != null) {
            try {
...
class LiquidityHunter(private val dqn: DQNTrader? = null) : BoardMember {
    ...
    override fun analyze(context: MarketContext): AgentOpinion {
        ...
        // V5.5.81: Blend with DQN learned capitulation/euphoria patterns
        val (finalSentiment, finalConfidence, dqnInsight) = if (dqn != null) {
            try {
 ...
 * 
 * V5.5.52: New board member for Global Macro strategy integration.
 * Uses real-time data from MacroSentimentAnalyzer when available.
 */
// ============================================================================
// AI BOARD ORCHESTRATOR - V5.5.62 OCTAGON (8-Member Core Board)
// ============================================================================
//
 ...
 * V5.5.80: Now accepts optional DQN learning engine to provide learned insights
 */
class AIBoardOrchestrator(
     ...
     * 
     * V5.5.80: Board members now receive DQN instance to blend learned patterns
     ...
     * 
     * V5.6.0: Now accepts regime-aware weight overrides from TradingCoordinator.
    ...
    fun conveneBoardroom(
        ...
        // Calculate weighted score — V5.6.0: use regime-aware weights when available
        ...
        // V5.6.0: Calculate recommended position size multiplier (0.0–1.0)
    ...
    /**
     * V5.6.0: Board-level position size recommendation.
     * 
...
private fun sentimentToVote(sentiment: Double): BoardVote {
    return when {
