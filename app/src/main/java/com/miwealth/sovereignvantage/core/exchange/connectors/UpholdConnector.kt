...
class UpholdConnector(
    ...
    override fun parseTicker(response: JsonObject, symbol: String): PriceTick? {
        ...
        return PriceTick(symbol = normaliseSymbol(symbol), bid = bid, ask = ask, last = (bid+ask)/2,
            timestamp = System.currentTimeMillis(), exchange = "uphold")
    }
    ...
    override fun parseTradingPairs(response: JsonObject): List<TradingPair> {
        ...
        FOREX_PAIRS.forEach { pair ->
            ...
            pairs.add(TradingPair(
                symbol = pair, baseAsset = b, quoteAsset = q,
                exchangeSymbol = "$b-$q", exchange = "uphold",
                minQuantity = 1.0, maxQuantity = 1e9,
                quantityPrecision = 2, pricePrecision = 5,
                minNotional = 1.0, isActive = true
            ))
        }
        // Add metal pairs
        METALS.forEach { m ->
            listOf("USD", "EUR", "GBP").forEach { f ->
                pairs.add(TradingPair(
                    symbol = "$m/$f", baseAsset = m, quoteAsset = f,
                    exchangeSymbol = "$m-$f", exchange = "uphold",
                    minQuantity = 0.001, maxQuantity = 1e9,
    ...
    override fun parseBalances(response: JsonObject): List<Balance> {
        return response.getAsJsonArray("cards")?.mapNotNull { c ->
            ...
            if (bal > 0) Balance(cur, avail, bal - avail, bal) else null
        } ?: emptyList()
    }
    ...
    override fun parseOrder(response: JsonObject): ExecutedOrder? {
        ...
        return ExecutedOrder(
            orderId = id, clientOrderId = "", symbol = "$oCur/$dCur",
            side = TradeSide.SELL, type = OrderType.MARKET,
            status = when(status) { "completed" -> OrderStatus.FILLED; "cancelled" -> OrderStatus.CANCELLED; else -> OrderStatus.OPEN },
            price = if (oAmt > 0) dAmt/oAmt else 0.0,
            quantity = oAmt,
            executedQuantity = if (status == "completed") oAmt else 0.0,
            executedPrice = if (oAmt > 0) dAmt/oAmt else 0.0,
            fee = 0.0, feeCurrency = dCur,
            timestamp = System.currentTimeMillis(), updatedAt = System.currentTimeMillis(),
            exchange = "uphold"
        )
    }
    ...
    override suspend fun getTicker(symbol: String): PriceTick? {
        ...
        return getRate(b, q)?.let { PriceTick(symbol = symbol, bid = it.bid, ask = it.ask, last = it.mid, timestamp = it.timestamp, exchange = "uphold") }
    }
    ...
    suspend fun executeForexTrade(from: String, to: String, amount: Double): ForexTradeResult {
        return when (val r = placeOrder(OrderRequest("$from/$to", TradeSide.SELL, OrderType.MARKET, amount))) {
            ...
            is OrderExecutionResult.Error -> ForexTradeResult.Failure(r.exception.message ?: "Failed")
