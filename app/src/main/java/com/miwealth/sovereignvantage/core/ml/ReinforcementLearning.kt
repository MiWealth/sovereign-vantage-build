 ...
 * V5.5.80: Added target network for training stability
 */
/**
 * Deep Q-Network (DQN) Trader with Continuous Features
 * 
 * V5.5.83: Enhanced with:
 ...
 */
class DQNTrader(
    private val stateSize: Int = 30,               // V5.5.83: Increased from 6 to 30 (normalized + interactions)
    ...
    // V5.5.89: DQN-eligible actions (first 5 — CLOSE_ALL is emergency-only, not DQN-driven)
    ...
    private val featureNormalizer = FeatureNormalizer()  // V5.5.83: Z-score normalization + interactions
    private var totalReward = 0.0
    private var episodeCount = 0
    private var stepCount = 0
    
    // V5.5.83: Experience now uses continuous normalized features instead of discretized state
    ...
    /**
     * Selects action using epsilon-greedy policy
     * V5.5.83: Now accepts EnhancedFeatureVector with continuous normalized features
     */
    fun selectAction(features: EnhancedFeatureVector, currentPosition: Double): TradingAction {
        return if (Random.nextDouble() < explorationRate) {
            dqnActions.random()  // V5.5.89: Only explore DQN-eligible actions (not CLOSE_ALL)
        } else {
     ...
     * V5.5.83: Uses normalized continuous features (30D) instead of discretized state (6D)
     */
    fun getBestAction(features: EnhancedFeatureVector, currentPosition: Double): TradingAction {
        ...
        return dqnActions[maxIndex]  // V5.5.89: Map to DQN-eligible actions only
    }
    
    /**
     * Stores experience in replay buffer
     * V5.5.83: Uses normalized features instead of discretized state
     */
    fun remember(
     ...
     * V5.5.80: Uses separate target network for stable Q-value targets
     * V5.5.83: Works with continuous normalized features (30D)
     */
    fun replay(batchSize: Int = 32) {
        ...
        batch.forEach { experience ->
            // V5.5.83: Features are already normalized DoubleArray
            ...
            if (actionIndex < 0) continue  // V5.5.89: Skip CLOSE_ALL (not in DQN action space)
     ...
     * V5.5.83: Allows AI Board to keep using MarketState while DQN uses continuous features
     * 
     ...
     * V5.5.80: Added for AI Board integration - board members query DQN's learned patterns
     * V5.5.83: Updated to use continuous normalized features
     * 
     ...
     * V5.5.80: Added for AI Board integration
     * V5.5.83: Updated to use continuous normalized features
     * 
    ...
    // ========================================================================
    // V5.5.89: Direct EnhancedFeatureVector Query Methods
     ...
     * V5.5.89: Avoids the lossy MarketState discretization.
     ...
     * V5.5.89: Richer signal than the MarketState path (30 real features vs 6 reconstructed).
     * 
     ...
     * V5.5.89: Uses all 30 features for more accurate sentiment vs 6 discretized.
     * 
     ...
     * V5.5.88: Fixes missing method referenced by TradingCoordinator.
 ...
 * 
 * V5.5.84: Upgraded to 2 hidden layers for hierarchical feature learning
...
class SimpleNeuralNetwork(
    ...
    private val hidden2Size: Int = 32               // V5.5.84: Second hidden layer
) {
    ...
    // Layer 2: Hidden1 → Hidden2 (V5.5.84: NEW)
     ...
     * V5.5.84: Added second hidden layer for hierarchical learning
     */
     ...
     * V5.5.80: Fixed to update ALL weights
     * V5.5.84: Extended to 2 hidden layers
     */
     ...
     * V5.5.80: Added for EWC Fisher Information calculation
     * V5.5.84: Extended to 2 hidden layers
    ...
    fun computeGradients(input: List<Double>, target: List<Double>): List<Double> {
        ...
        // weightsHidden1Hidden2 gradients (V5.5.84: NEW)
        ...
        // biasHidden2 gradients (V5.5.84: NEW)
     ...
     * V5.5.80: Added for target network synchronization and federated learning
     * V5.5.84: Extended to 2-layer architecture
    ...
    fun getAllWeights(): List<Double> {
        ...
        // Flatten weightsHidden1Hidden2 [hidden1Size][hidden2Size] (V5.5.84: NEW)
        for (h1 in 0 until hidden1Size) {
            for (h2 in 0 until hidden2Size) {
        ...
        // Add biasHidden2 [hidden2Size] (V5.5.84: NEW)
     ...
     * V5.5.80: Added for target network synchronization and DFLP weight injection
     * V5.5.84: Extended to 2-layer architecture
     */
    fun setAllWeights(weights: List<Double>) {
        ...
        // Reconstruct weightsHidden1Hidden2 (V5.5.84: NEW)
        ...
        // Reconstruct biasHidden2 (V5.5.84: NEW)
     ...
     * V5.5.80: Added for DQN target network updates
     */
