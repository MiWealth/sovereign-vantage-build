 ...
 * SENTIMENT ENGINE
 * 
 * Sovereign Vantage: Arthur Edition V5.5.93
...
data class SentimentScore(
    ...
    val providerName: String = "unknown", // V5.5.91: Which provider supplied the data
    val providerConfidence: Double = 1.0  // V5.5.91: Provider's confidence in score
)

/**
 * The specialist engine for real-time social media sentiment analysis.
 * 
 * V5.5.92: Singleton pattern — use SentimentEngine.getInstance(context) to
...
class SentimentEngine(
    ...
    // V5.5.92: Reference counting for singleton lifecycle management.
    ...
    private suspend fun analyzeSentiment(asset: String): SentimentScore {
        ...
        val macroAdj = cachedMacroContext?.let { macro ->
            when (macro.overallSentiment) {
                SentimentDirection.HAWKISH -> -0.15 * macro.confidence
                SentimentDirection.DOVISH -> 0.15 * macro.confidence
    ...
    companion object {
         ...
         * 
         * V5.5.92: All consumers share one instance to avoid:
