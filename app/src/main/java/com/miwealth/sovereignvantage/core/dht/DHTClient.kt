...
class DHTClient(
    ...
    /**
     * Publish a serializable value to DHT (convenience wrapper around put).
     * Serialises via JSON before storing as bytes.
     */
    suspend fun publish(key: String, value: Any, replicationNodes: Int = REPLICATION_FACTOR): Boolean {
        val bytes = value.toString().toByteArray(Charsets.UTF_8)
        return put(key, bytes, replicationNodes)
    }

    /**
     * Retrieve a batch of keys in one call.
     */
    suspend fun queryBatch(keys: List<String>): Map<String, ByteArray?> {
        return keys.associateWith { get(it) }
    }

    /**
     * Retrieve a typed batch from DHT by prefix key + count.
     * Returns empty list until full serialization is implemented.
     */
    @Suppress("UNCHECKED_CAST")
    suspend fun <T> queryBatch(prefixKey: String, batchSize: Int): List<T> {
        // Stub: full typed deserialization wired in V2
        return emptyList()
    }
}
