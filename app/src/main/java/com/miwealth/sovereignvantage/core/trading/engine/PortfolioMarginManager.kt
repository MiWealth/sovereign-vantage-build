...
sealed class PortfolioEvent {
    ...
    data class PositionOpened(val position: PositionSummary) : PortfolioEvent()
    data class PositionClosed(val symbol: String, val realizedPnl: Double) : PortfolioEvent()
    data class PositionUpdated(val position: PositionSummary) : PortfolioEvent()
...
class PortfolioMarginManager(
    ...
    private val positionCache = mutableMapOf<String, PositionSummary>()
    ...
    private suspend fun fetchBalances(): List<AssetBalance> {
        ...
        aiConnector?.let { connector ->
            try {
                val balanceMap = connector.getBalances()
                return balanceMap.map { (asset, balance) ->
                    AssetBalance(
                        asset = asset,
                        free = balance.free,
                        locked = balance.locked,
                        total = balance.total,
                        usdValue = estimateUsdValue(asset, balance.total)
                    )
                }
            } catch (e: Exception) {
        ...
        exchangeConnector?.let { connector ->
            val balanceMap = connector.getAccountBalances()
            return balanceMap.map { (asset, amounts) ->
                val free = amounts["free"] ?: 0.0
                val locked = amounts["locked"] ?: 0.0
                ...
                AssetBalance(
                    ...
                    locked = locked,
                    total = total,
                    usdValue = estimateUsdValue(asset, total)
                )
            }
        }
     ...
     */
    private suspend fun fetchPositions(): List<PositionSummary> {
        // Try AI connector first
        aiConnector?.let { connector ->
            try {
                val openOrders = connector.getOpenOrders()
                // Convert filled orders to positions (simplified)
                return openOrders.filter { it.status == "filled" || it.status == "open" }
                    .map { order ->
                        ...
                        PositionSummary(
                            symbol = order.symbol,
                            side = if (order.side == "buy") PositionSide.LONG else PositionSide.SHORT,
     ...
     */
    private fun calculateUnrealizedPnl(position: PositionSummary, currentPrice: Double): Double {
