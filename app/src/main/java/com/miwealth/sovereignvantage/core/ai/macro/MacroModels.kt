...
/**
 * SOVEREIGN VANTAGE V5.5.51 - MACRO SENTIMENT MODELS
...
class MacroTypeConverters {
    @TypeConverter fun fromCentralBank(v: CentralBank?): String? = v?.name
    @TypeConverter fun toCentralBank(v: String?): CentralBank? = v?.let { CentralBank.valueOf(it) }
    @TypeConverter fun fromIndicatorType(v: IndicatorType?): String? = v?.name
    @TypeConverter fun toIndicatorType(v: String?): IndicatorType? = v?.let { IndicatorType.valueOf(it) }
    @TypeConverter fun fromSentimentDirection(v: SentimentDirection?): String? = v?.name
    @TypeConverter fun toSentimentDirection(v: String?): SentimentDirection? = v?.let { SentimentDirection.valueOf(it) }
    @TypeConverter fun fromImpactLevel(v: ImpactLevel?): String? = v?.name
    @TypeConverter fun toImpactLevel(v: String?): ImpactLevel? = v?.let { ImpactLevel.valueOf(it) }
    @TypeConverter fun fromEventStatus(v: EventStatus?): String? = v?.name
    @TypeConverter fun toEventStatus(v: String?): EventStatus? = v?.let { EventStatus.valueOf(it) }
}
