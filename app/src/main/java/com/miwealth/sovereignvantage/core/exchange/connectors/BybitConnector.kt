...
class BybitConnector(
    ...
    override fun signRequest(
        ...
        val signPayload = "$timestamp${config.credentials?.apiKey}$recvWindow$paramString"
        
        // HMAC-SHA256 signature
        val signature = hmacSha256(signPayload, config.credentials?.apiSecret ?: "")
        
        return mapOf(
            "X-BAPI-API-KEY" to (config.credentials?.apiKey ?: ""),
        ...
        return try {
            ...
            list.mapNotNull { item ->
                ...
                TradingPair(
                    ...
                    status = if (obj.get("status")?.asString == "Trading") "ACTIVE" else "INACTIVE",
    ...
    private fun parseOrderFromObject(obj: JsonObject): ExecutedOrder {
        ...
        return ExecutedOrder(
            ...
            requestedPrice = obj.get("price")?.asString?.toDoubleOrNull() ?: 0.0,
            executedPrice = obj.get("avgPrice")?.asString?.toDoubleOrNull() ?: 0.0,
            requestedQuantity = obj.get("qty")?.asString?.toDoubleOrNull() ?: 0.0,
    ...
    override fun parsePlaceOrderResponse(json: JsonObject): OrderExecutionResult {
        return try {
            ...
            if (retCode != 0) {
                val msg = json.get("retMsg")?.asString ?: "Unknown error"
                return OrderExecutionResult.Rejected(msg, retCode.toString())
            }
            
            val result = json.getAsJsonObject("result")
            val orderId = result?.get("orderId")?.asString ?: ""
            
            // Bybit returns minimal info on order creation, need to query for full details
    ...
    override fun buildOrderBody(request: OrderRequest): String {
        val jsonObj = JsonObject().apply {
            addProperty("category", "spot")
            addProperty("symbol", toExchangeSymbol(request.symbol))
            addProperty("side", if (request.side == TradeSide.BUY || request.side == TradeSide.LONG) "Buy" else "Sell")
            addProperty("orderType", when (request.type) {
            ...
            addProperty("qty", request.quantity.toString())
            
            if (request.type == OrderType.LIMIT || request.type == OrderType.STOP_LIMIT) {
                request.price?.let { addProperty("price", it.toString()) }
            }
            
            if (request.type == OrderType.STOP_LOSS || request.type == OrderType.STOP_LIMIT) {
                request.stopPrice?.let { addProperty("triggerPrice", it.toString()) }
            }
            
            addProperty("timeInForce", when (request.timeInForce) {
                TimeInForce.GTC -> "GTC"
                TimeInForce.IOC -> "IOC"
                TimeInForce.FOK -> "FOK"
                else -> "GTC"
            ...
            if (request.clientOrderId.isNotEmpty()) {
                addProperty("orderLinkId", request.clientOrderId)
            }
            
            if (request.reduceOnly) {
                addProperty("reduceOnly", true)
            }
        }
    ...
    override fun getTradingPairsEndpoint(): String = "/v5/market/instruments-info?category=spot"
    
    override fun getBalancesEndpoint(): String = "/v5/account/wallet-balance?accountType=UNIFIED"
    
    override fun getPlaceOrderEndpoint(): String = "/v5/order/create"
    
    override fun getCancelOrderEndpoint(orderId: String, symbol: String): String = "/v5/order/cancel"
