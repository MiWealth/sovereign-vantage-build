...
/**
 * SOVEREIGN VANTAGE V5.6.0 "ARTHUR EDITION"
...
data class HybridModeConfig(
    ...
    // ---- V5.5.97: Confidence-Based Gates (from SettingsViewModel) ----
...
data class CoordinatorStatus(
    ...
    val mlHealthStatus: HealthStatus = HealthStatus.HEALTHY,
    val mlRollbackCount: Int = 0,
    // V5.5.88: Disagreement detection
    val disagreementLevel: String = "STRONG_AGREEMENT",
    val positionSizeMultiplier: Double = 1.0
)
...
class TradingCoordinator(
    ...
    // V5.5.90: Sentiment Engine — provides socialVolume + newsImpact to AI Board
    private val sentimentEngine: SentimentEngine? = null
) {
    
    // AI Systems
    // V5.5.80: DQN Learning Engine - teaches board members what actually works
    // V5.5.89: stateSize upgraded from 6 → 30 (uses v5.5.83 enhanced 30-feature vector)
    // 19 base features + 10 cross-feature interactions + 1 position = 30
    private val dqn = DQNTrader(
        stateSize = 30,          // V5.5.83 EnhancedFeatureVector (was 6 prior to v5.5.89)
    ...
    // V5.5.88: Health monitor for DQN — tracks gradient/weight/loss health
    // V5.5.89: Dimensions match the DQN's upgraded network (30→64→32→5)
    ...
    // V5.5.86/88: Ensemble disagreement detector — reduces position size when models conflict
    private val disagreementDetector = EnsembleDisagreementDetector()
    
    // V5.5.80: AI Board now receives DQN to blend learned patterns with heuristics
    ...
    // V5.5.97: Dynamic Board Weights by Market Regime
    ...
    // V5.5.97: Cross-exchange price map for arb/hedge strategies
    ...
    // V5.6.0: Order book snapshots per symbol per exchange for real bid/ask depth
    ...
    // V5.5.97: HYBRID mode rate limiting — separate from global rate limits
    ...
    fun start() {
        ...
        // V5.5.88: Checkpoint DQN weights at startup as baseline rollback point
        try {
    ...
    /**
     * V5.5.97: Exchange-aware price tick handler.
    ...
    fun onPriceTick(symbol: String, price: Double, volume: Double, exchange: String?) {
        ...
        // V5.5.97: Update cross-exchange price map for arb/hedge strategies
        if (exchange != null) {
    ...
    /**
     * V5.6.0: Order book update handler.
    ...
    /**
     * V5.6.0: Get the latest order book for a symbol on a specific exchange.
    ...
    /**
     * V5.6.0: Get all order books for a symbol across exchanges.
    ...
    /**
     * V5.5.97: Get cross-exchange prices for a symbol.
     * Returns a map of exchangeId → latest PriceTick.
     * Used by arb/hedge strategies to detect spread opportunities.
     *
     * V5.6.0: PriceTick bid/ask now populated from real order book data
    ...
    /**
     * V5.5.97: Calculate the current arb spread for a symbol across exchanges.
     * Returns the percentage spread between lowest ask and highest bid across venues.
     * Positive spread = potential arb opportunity.
     *
     * V5.6.0: Now uses real best-bid/best-ask from order book depth data
    ...
    /**
     * V5.5.88: Get current ML health report for UI display.
    ...
    private suspend fun analyzeSymbol(symbol: String, buffer: PriceBuffer) {
        ...
        // V5.5.90: Enrich with SentimentEngine data if available
        ...
        // V5.5.97: Update market regime BEFORE board convenes — this ensures
        ...
        // V5.6.0: Build regime-aware weight overrides for the board.
        ...
        // V5.5.88: Run disagreement analysis on board opinions
        ...
        // Update state with disagreement info and V5.6.0 board position sizing
        ...
        // V5.5.88: Gate check — skip trade if confidence doesn't meet disagreement-adjusted threshold
    ...
    private suspend fun handleHybridMode(signal: PendingTradeSignal) {
        ...
        // V5.5.97: Reset hybrid hourly counter if needed
        ...
        // V5.5.97: LAYERED HYBRID DECISION FLOW
    ...
    private suspend fun executeTrade(signal: PendingTradeSignal): Result<ExecutedTrade> {
        ...
        return try {
            ...
            val result = if (config.paperTradingMode) {
                simulatePaperTrade(signal, quantity)
            } else if (config.useStahlStops) {
    ...
    private suspend fun handleSuccessfulTrade(
        ...
        // Update gamification
        gamification?.onTradeOpened(signal.symbol, executedTrade.entryPrice, executedTrade.quantity)
    ...
    private fun simulatePaperTrade(signal: PendingTradeSignal, quantity: Double): OrderExecutionResult {
        return OrderExecutionResult.Success(
            ExecutedOrder(
                orderId = "PAPER-${System.currentTimeMillis()}",
                clientOrderId = "SV-PAPER-${System.currentTimeMillis()}",
                symbol = signal.symbol,
                side = if (signal.direction == TradeDirection.LONG) TradeSide.BUY else TradeSide.SELL,
                type = OrderType.MARKET,
                requestedPrice = signal.suggestedEntry,
                executedPrice = signal.suggestedEntry,
                requestedQuantity = quantity,
                executedQuantity = quantity,
                fee = quantity * signal.suggestedEntry * 0.001,
                feeCurrency = "USD",
    ...
    private suspend fun closePositionOnStop(symbol: String, position: ManagedPosition, reason: String) {
        ...
        when (result) {
            is OrderExecutionResult.Success, is OrderExecutionResult.PartialFill -> {
                ...
                // V5.5.80: Train DQN on trade outcome
                // DQN learns from actual profit/loss to improve future predictions
                // V5.5.88: Now monitored by DQNHealthMonitor with auto-rollback
                try {
                    ...
                    // V5.5.88: Train with metrics and monitor health
                ...
                // Update gamification
                gamification?.onTradeClosed(symbol, pnl, pnlPercent)
     ...
     */
    /**
     * V5.6.0: Build a complete regime-aware weight override map for all 8 board members.
    ...
    /**
     * V5.5.97: Compute volatility history from PriceBuffer for MarketRegimeDetector.
     * 
    ...
    /**
     * V5.5.97: Update current market regime from price data.
        ...
        withContext(Dispatchers.IO) {
            try {
                val trade = TradeEntity(
                    ...
                    signalConfidence = signal.confidence.toInt(),
                    aiScore = (signal.boardConsensus.confidence * 100).toInt(),
    ...
    private fun generateTradeSignal(
        ...
        // V5.6.0: POSITION SIZING HIERARCHY
