...
class StrategyRiskManager(
        ...
        monitoringJob = scope.launch {
            while (isActive) {
                try {
                    ...
                    val portfolioValue = summary.totalValue
    ...
    private suspend fun liquidateAllPositions(): List<LiquidationResult> {
        ...
        val positions = positionManager.getOpenPositions()
        ...
        for (position in positions) {
            ...
            try {
                _events.emit(RiskManagerEvent.LiquidatingPosition(
                    ...
                    estimatedValue = position.unrealizedPnl + position.entryPrice * position.quantity
                ))
                ...
                when (result) {
                    is OrderExecutionResult.Success -> {
                        val slippage = abs(result.averagePrice - preLiquidationPrice) / preLiquidationPrice * 100
                        ...
                        results.add(LiquidationResult(
                            ...
                            stablecoinReceived = result.executedQuantity * result.averagePrice,
                            success = true
                        ))
                    }
                    is OrderExecutionResult.Failed -> {
                        results.add(LiquidationResult(
                            symbol = position.symbol,
                            requestedQuantity = position.quantity,
