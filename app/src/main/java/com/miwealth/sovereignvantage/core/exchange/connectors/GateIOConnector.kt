...
class GateIOConnector(
    ...
    override val capabilities = ExchangeCapabilities(
        ...
        supportedTimeInForce = listOf(
            TimeInForce.GTC,
            TimeInForce.IOC,
            TimeInForce.GTD  // Pending or Cancelled
        )
    )
    ...
    override fun parseOrder(response: JsonObject): ExecutedOrder? {
        return try {
            ...
            ExecutedOrder(
                ...
                price = price,
                executedPrice = if (filledAmount > 0) fillPrice else price,
                quantity = amount,
    ...
    override fun buildOrderBody(request: OrderRequest): Map<String, String> {
        ...
        // Iceberg order
        // Iceberg/hidden orders not in base OrderRequest
