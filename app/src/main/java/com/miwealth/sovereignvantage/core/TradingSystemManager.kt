...
// V5.5.78: ExchangeCredentials unified - single definition in core.exchange package
...
class TradingSystemManager @Inject constructor(
    ...
    suspend fun initializeAILiveTrading(
        ...
        return try {
            ...
            val config = TradingSystemConfig(
                ...
                primaryCredentials = credentials  // V5.6.1: Pass creds so connector is created WITH auth
            )
            
            val result = aiIntegratedSystem!!.initialize(config)
            
            // V5.6.1: Credentials now flow through config → setupLiveExchange() → addKnownExchange()
    ...
    suspend fun initializeTestnetTrading(
        ...
        return try {
            ...
            val config = TradingSystemConfig(
                ...
                isTestnetMode = true,  // V5.5.97: Flag for UI and safety checks
                tradingSymbols = tradingSymbols,
                useStahlStops = true,
                enableAssetDiscovery = true,
     ...
     * 
     * V5.5.97: For arb/hedge strategy testing across testnet exchanges.
     ...
     * 
     * V5.5.94 CHANGE (Gap 1 fix): When USE_AI_INTEGRATION is true, routes
    ...
    suspend fun initializeWithCredentials(
        ...
        // V5.5.94: Route through AI path if enabled (BRIDGE — legacy preserved below)
        if (USE_AI_INTEGRATION) {
     ...
     * 
     * V5.5.94 SCAFFOLD (Gap 2): When USE_AI_INTEGRATION is true, routes through
    ...
    suspend fun initializeWithPQC(
        ...
        // V5.5.97: Route through AI path if enabled (BRIDGE — legacy preserved below)
        if (USE_AI_INTEGRATION) {
            ...
            // 2. V5.5.97 (Gap 2 complete): Register ALL remaining exchanges
    ...
    private fun updateDashboardFromAIState(state: IntegratedTradingState) {
        _dashboardState.update { current ->
            current.copy(
                ...
                // V5.5.97: Testnet mode propagation to UI
                isTestnetMode = state.isTestnetMode,
                // V5.5.89: ML Health + Disagreement from CoordinatorState
    ...
    private fun handleCoordinatorEvent(event: CoordinatorEvent) {
        when (event) {
            ...
            // V5.5.89: Bridge ML health events from TradingCoordinator to DashboardState → ViewModel → UI
    ...
    /**
     * V5.5.97: Remove an exchange from the trading system.
    ...
    fun getRegisteredAssetCount(): Int {
        return com.miwealth.sovereignvantage.core.trading.assets.AssetRegistry.getAllAssets().size
    }
