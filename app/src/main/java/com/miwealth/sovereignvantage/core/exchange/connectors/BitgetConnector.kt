    ...
    override fun parseOrder(response: JsonObject): ExecutedOrder? {
        return try {
            ...
            ExecutedOrder(
                ...
                executedQuantity = data.get("baseVolume")?.asString?.toDoubleOrNull() ?: 0.0,
                executedPrice = data.get("priceAvg")?.asString?.toDoubleOrNull() ?: 0.0,
                fee = data.get("fee")?.asString?.toDoubleOrNull() ?: 0.0,
                feeCurrency = data.get("feeCcy")?.asString ?: "",
                timestamp = data.get("cTime")?.asLong ?: System.currentTimeMillis(),
