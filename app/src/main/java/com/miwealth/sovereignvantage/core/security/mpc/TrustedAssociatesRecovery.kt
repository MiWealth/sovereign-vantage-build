...
class TrustedAssociatesRecovery(
    ...
    suspend fun designateTrustedContacts(
        ...
        try {
            ...
            secureStorage.storeMetadata("trusted_contacts_$walletId", metadata)
    ...
    suspend fun acceptRecoveryInvitation(
        ...
        try {
            ...
            val encryptedBackup = pqCrypto.encrypt(decryptedShare, secureStorage.getPublicKey(userId))
    ...
    suspend fun requestRecovery(
        ...
        try {
            ...
            val metadata = secureStorage.retrieveMetadata<TrustedContactsMetadata>(
                "trusted_contacts_$walletId"
            ...
            val recoveryRequest = RecoveryRequest(
                ...
                expiresAt = System.currentTimeMillis() + (RECOVERY_REQUEST_TIMEOUT_HOURS * 60 * 60 * 1000),
                timestamp = System.currentTimeMillis()
            )
            
            // Send request to all contacts via DHT
            metadata.contacts.forEach { contactId ->
            ...
            val responses = collectRecoveryResponses(
                ...
                metadata.threshold,
                RECOVERY_REQUEST_TIMEOUT_HOURS * 60 * 60 * 1000
            )
    ...
    suspend fun revokeTrustedContact(
        ...
        try {
            ...
            secureStorage.storeMetadata("trusted_contacts_$walletId", newMetadata)
    ...
    private suspend fun checkRecoveryRateLimit(): Boolean {
        ...
        val count = secureStorage.retrieveMetadata<Int>(key) ?: 0
        
        if (count >= MAX_RECOVERY_REQUESTS_PER_DAY) {
            return false
        }
        
        secureStorage.storeMetadata(key, count + 1)
        return true
    }
    ...
    private suspend fun recordRecoveryEvent(
        ...
        // Store locally
        secureStorage.appendToLog("recovery_events", event)
