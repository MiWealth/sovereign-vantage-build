...
class KuCoinConnector(
    ...
    override val capabilities = ExchangeCapabilities(
        ...
        supportedTimeInForce = listOf(
            ...
            TimeInForce.IOC,
            TimeInForce.FOK,
            TimeInForce.GTD  // Good Till Time
        )
    )
    ...
    override fun parseOrder(response: JsonObject): ExecutedOrder? {
        return try {
            ...
            val timestamp = data.get("createdAt")?.asLong ?: System.currentTimeMillis()
            ...
            ExecutedOrder(
                ...
                price = price,
                executedPrice = if (dealSize > 0) dealFunds / dealSize else price,
                quantity = size,
    ...
    override fun buildOrderBody(request: OrderRequest): Map<String, String> {
        ...
        // Hidden/iceberg orders not supported in base OrderRequest
    ...
    suspend fun getWebSocketToken(): String? {
        val responseStr = executeGet("/api/v1/bullet-public") ?: return null
        val response = try { gson.fromJson(responseStr, JsonObject::class.java) } catch (e: Exception) { return null }
        val data = response.getAsJsonObject("data") ?: return null
    ...
    suspend fun getRecentFills(symbol: String? = null, limit: Int = 50): List<ExecutedOrder> {
        ...
        return data.mapNotNull { item ->
            ...
            ExecutedOrder(
                ...
                executedPrice = obj.get("price")?.asString?.toDoubleOrNull() ?: 0.0,
                quantity = obj.get("size")?.asString?.toDoubleOrNull() ?: 0.0,
