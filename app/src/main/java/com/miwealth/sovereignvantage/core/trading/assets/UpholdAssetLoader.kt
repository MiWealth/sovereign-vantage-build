...
class UpholdAssetLoader(
    ...
    override val exchangeId: String = "uphold"
    override val exchangeName: String = "Uphold"
    
    // =========================================================================
    // PUBLIC API
    // =========================================================================
    
    override suspend fun loadAllAssets(): List<CryptoAsset> {
    ...
    override suspend fun loadAsset(symbol: String): CryptoAsset? {
        return withContext(Dispatchers.IO) {
            val upholdAssets = fetchAssets()
    ...
    override suspend fun getTradingPairs(): List<TradingPairInfo> {
        return withContext(Dispatchers.IO) {
    ...
    override suspend fun refreshAssets(): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                cachedAssets = null
                cacheTimestamp = 0
    ...
    suspend fun loadCryptoAssets(): List<CryptoAsset> {
        return loadAllAssets().filter { 
            it.category != CryptoCategory.STABLECOIN &&
     ...
     */
    suspend fun loadForexAssets(): List<ForexAsset> {
        return withContext(Dispatchers.IO) {
            val upholdAssets = fetchAssets()
            upholdAssets
    ...
    private fun convertToTradableAsset(asset: UpholdAsset): CryptoAsset? {
        ...
        val category = inferCategory(asset.code)
        val riskParams = getRiskParamsForCategory(category)
        
        return CryptoAsset(
            symbol = asset.code,
            name = asset.name,
            category = category,
            exchanges = listOf("UPHOLD"),
            // Risk parameters
            kellyMultiplier = riskParams.kelly,
            maxPositionPercent = riskParams.maxPosition,
            maxLeverage = riskParams.leverage,
            sacredStopLoss = SACRED_STOP_LOSS,  // Always 3.5%
            // Market data placeholders (would be populated from tickers)
            marketCapUsd = null,
            volume24hUsd = null,
    ...
    private fun convertToForexAsset(asset: UpholdAsset): ForexAsset {
        return ForexAsset(
            symbol = asset.code,
            name = asset.name,
