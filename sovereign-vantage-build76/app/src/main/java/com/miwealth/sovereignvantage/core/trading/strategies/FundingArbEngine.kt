...
data class FundingArbConfig(
    ...
    /** V5.17.0: Minimum cross-exchange arb spread (%) to justify entry.
...
class FundingArbEngine(
    ...
    /** V5.17.0: Optional coordinator for cross-exchange arb spread awareness.
     ...
     *
     * V5.17.0: Added cross-exchange arb spread gate. If TradingCoordinator is
    ...
    private suspend fun evaluateEntry(symbol: String, fundingData: FundingRateData) {
        ...
        // V5.17.0: Cross-exchange arb spread gate
    ...
    suspend fun executeArb(symbol: String, notionalValue: Double) {
        ...
        try {
            ...
            if (spotResult !is OrderExecutionResult.Success) {
                throw Exception("Spot order failed: ${(spotResult as? OrderExecutionResult.Rejected)?.reason}")
            }
            
            val actualSpotQty = spotResult.order.executedQuantity
            val actualSpotPrice = spotResult.order.executedPrice
            ...
            if (perpResult !is OrderExecutionResult.Success) {
                ...
                throw Exception("Perp order failed: ${(perpResult as? OrderExecutionResult.Rejected)?.reason}")
            }
            
            val actualPerpQty = -perpResult.order.executedQuantity  // Negative for short
            val actualPerpPrice = perpResult.order.executedPrice
     ...
     *
     * V5.17.0: When TradingCoordinator is available with cross-exchange order book
    ...
    private suspend fun checkAllPositionDeltas() {
        for ((symbol, position) in _activePositions.value) {
            try {
                // V5.17.0: Try cross-exchange prices first (real bid/ask from order books)
