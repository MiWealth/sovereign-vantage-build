...
class HTXConnector(
    ...
    override fun parseTicker(response: JsonObject, symbol: String): PriceTick? {
        ...
        return PriceTick(symbol = normaliseSymbol(symbol), bid = t.getAsJsonArray("bid")?.get(0)?.asDouble ?: 0.0,
            ask = t.getAsJsonArray("ask")?.get(0)?.asDouble ?: 0.0, last = t.get("close")?.asDouble ?: 0.0,
            volume = t.get("vol")?.asDouble ?: 0.0, high24h = t.get("high")?.asDouble ?: 0.0, low24h = t.get("low")?.asDouble ?: 0.0,
            timestamp = System.currentTimeMillis(), exchange = "htx")
    }
    
    override fun parseOrderBook(response: JsonObject, symbol: String): OrderBook? {
        val t = response.getAsJsonObject("tick") ?: return null
        return OrderBook(symbol = normaliseSymbol(symbol), exchange = "htx",
            bids = t.getAsJsonArray("bids")?.map { OrderBookLevel(it.asJsonArray[0].asDouble, it.asJsonArray[1].asDouble) } ?: emptyList(),
            asks = t.getAsJsonArray("asks")?.map { OrderBookLevel(it.asJsonArray[0].asDouble, it.asJsonArray[1].asDouble) } ?: emptyList(),
    ...
    override fun parseTradingPairs(response: JsonObject): List<TradingPair> {
        return response.getAsJsonArray("data")?.mapNotNull { s ->
            ...
            TradingPair(symbol = "$b/$q", baseAsset = b, quoteAsset = q,
                exchangeSymbol = o.get("symbol")?.asString ?: "", exchange = "htx",
                minQuantity = o.get("min-order-amt")?.asDouble ?: 0.0,
                maxQuantity = o.get("max-order-amt")?.asDouble ?: 1e9,
                quantityPrecision = o.get("amount-precision")?.asInt ?: 8,
                pricePrecision = o.get("price-precision")?.asInt ?: 8,
                minNotional = o.get("min-order-value")?.asDouble ?: 5.0)
        } ?: emptyList()
    }
    ...
    override fun parseBalances(response: JsonObject): List<Balance> {
        ...
        return m.filter { it.value.first + it.value.second > 0 }
            .map { Balance(it.key, it.value.first, it.value.second, it.value.first + it.value.second) }
    }
    ...
    override fun parseOrder(response: JsonObject): ExecutedOrder? {
        ...
        return ExecutedOrder(
            orderId = id, clientOrderId = d.get("client-order-id")?.asString ?: "",
            symbol = normaliseSymbol(sym),
            side = if (t.startsWith("buy")) TradeSide.BUY else TradeSide.SELL,
            type = if (t.contains("market")) OrderType.MARKET else OrderType.LIMIT,
            status = when (d.get("state")?.asString) { "filled" -> OrderStatus.FILLED; "canceled" -> OrderStatus.CANCELLED
                "partial-filled" -> OrderStatus.PARTIALLY_FILLED; else -> OrderStatus.OPEN },
            price = d.get("price")?.asString?.toDoubleOrNull() ?: 0.0,
            quantity = d.get("amount")?.asString?.toDoubleOrNull() ?: 0.0,
            executedQuantity = d.get("field-amount")?.asString?.toDoubleOrNull() ?: 0.0,
            timestamp = d.get("created-at")?.asLong ?: System.currentTimeMillis(), exchange = "htx")
    }
    ...
    override fun parsePlaceOrderResponse(response: JsonObject, request: OrderRequest): OrderExecutionResult {
        ...
        return OrderExecutionResult.Success(ExecutedOrder(orderId = id, clientOrderId = request.clientOrderId ?: "", symbol = request.symbol,
            side = request.side, type = request.type, status = OrderStatus.OPEN, price = request.price ?: 0.0,
            quantity = request.quantity, exchange = "htx"))
    }
    ...
    suspend fun placeFuturesOrder(req: FuturesOrderRequest): OrderExecutionResult {
        ...
        return try {
            ...
            else OrderExecutionResult.Success(ExecutedOrder(orderId = r.getAsJsonObject("data")?.get("order_id")?.asString ?: "",
                symbol = req.symbol, side = req.side, type = req.type, status = OrderStatus.OPEN,
                price = req.price ?: 0.0, quantity = req.quantity, feeCurrency = "USDT", exchange = "htx"))
        } catch (e: Exception) { OrderExecutionResult.Error(e) }
    }
